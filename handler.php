...
try {
    function mapCountry($countryName) {
        // Убираем флаг из названия страны
        $cleanCountry = preg_replace('/[^\p{L}\p{N}\s-]/u', '', $countryName);
        $cleanCountry = trim($cleanCountry);
    ...
    $leadFields = [
        ...
        'OPENED' => 'Y',
        'ASSIGNED_BY_ID' => 1,
        'UF_CRM_1753854417191' => $data['Тип продукта'] ?? '',
        'UF_CRM_1753854423190' => $data['пожелания'] ?? '',
        'UF_CRM_1753854458111' => $data['Бюджет'] ?? '',
        'UF_CRM_1753854481408' => mapCountry($data['страна'] ?? ''), // Вызываем функцию
    ...
    if (!empty($result['result'])) {
        ...
        //file_put_contents('debug_log.txt', date('Y-m-d H:i:s') . " - Country field: '" . ($data['страна'] ?? 'empty') . "' mapped to: '" . mapCountry($data['страна'] ?? '') . "'\n", FILE_APPEND);
